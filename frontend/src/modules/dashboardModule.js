...
export function renderDashboard() {
  ...
  return `
    <div class="dashboard-module">
      ...
      <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:12px; margin-bottom:20px; background:#ffffff; padding:16px 22px; border-radius:12px; border:1px solid #e2e8f0; box-shadow:0 1px 3px rgba(15,23,42,0.04);">
        <div>
          <h2 style="font-size:1.15rem; font-weight:800; color:#0f172a; letter-spacing:-0.02em;">Executive Asset Overview</h2>
          <span style="font-size:0.78rem; color:#64748b;">Real-time asset telemetry synchronized from Fixed Asset Register</span>
        </div>
        
        <div style="display:flex; align-items:center; gap:10px;">
          <label style="font-size:0.8rem; font-weight:700; color:#475569; display:flex; align-items:center; gap:6px;">
            <i data-lucide="filter" style="width:14px; height:14px; color:#4f46e5;"></i> Category Filter:
          </label>
          <select id="dashboardCategorySelect" class="select-control" style="width:240px; font-weight:600; padding:7px 12px; border-color:#cbd5e1; border-radius:8px;">
          ...
          ${selectedCategoryFilter ? `
            <button class="btn btn-outline btn-sm" id="btnResetDashCat" style="color:#4f46e5; border-color:#c7d2fe;">
      ...
      <div class="kpi-grid-mini" style="grid-template-columns: repeat(4, 1fr); margin-bottom: 22px;">
        ...
        <!-- 1. Total Assets Card -->
        <div class="kpi-mini-card" style="border-left: 4px solid #4f46e5; padding:18px 20px;">
          <div class="kpi-mini-info">
            <span class="kpi-mini-title" style="font-size:0.72rem; font-weight:700; color:#64748b;">TOTAL ASSETS</span>
            <span class="kpi-mini-val" style="color:#0f172a; font-size:1.65rem; font-weight:800; margin:4px 0;">${totalAssetsCount}</span>
            <span class="kpi-mini-sub" style="font-size:0.75rem; color:#64748b;">${allAssets.length} Total in Database</span>
          </div>
          <div style="width:46px; height:46px; border-radius:10px; background:#eef2ff; display:flex; align-items:center; justify-content:center;">
            <i data-lucide="boxes" style="color:#4f46e5; width:22px; height:22px;"></i>
          </div>
        </div>

        <!-- 2. Asset Value (Total Cost) Card -->
        <div class="kpi-mini-card" style="border-left: 4px solid #0284c7; padding:18px 20px;">
          <div class="kpi-mini-info">
            <span class="kpi-mini-title" style="font-size:0.72rem; font-weight:700; color:#64748b;">ASSET VALUE (TOTAL COST)</span>
            <span class="kpi-mini-val" style="color:#0284c7; font-size:1.5rem; font-weight:800; margin:4px 0;">${formatCurrency(totalGrossCost)}</span>
            <span class="kpi-mini-sub" style="font-size:0.75rem; color:#64748b;">Book Val: <strong>${formatCurrency(totalBookValue)}</strong></span>
          </div>
          <div style="width:46px; height:46px; border-radius:10px; background:#e0f2fe; display:flex; align-items:center; justify-content:center;">
            <i data-lucide="indian-rupee" style="color:#0284c7; width:22px; height:22px;"></i>
          </div>
        </div>

        <!-- 3. In Stock Card -->
        <div class="kpi-mini-card" style="border-left: 4px solid #059669; padding:18px 20px;">
          <div class="kpi-mini-info">
            <span class="kpi-mini-title" style="font-size:0.72rem; font-weight:700; color:#64748b;">IN STOCK (AVAILABLE)</span>
            <span class="kpi-mini-val" style="color:#059669; font-size:1.65rem; font-weight:800; margin:4px 0;">${inStockCount}</span>
          ...
          <div style="width:46px; height:46px; border-radius:10px; background:#ecfdf5; display:flex; align-items:center; justify-content:center;">
            <i data-lucide="archive" style="color:#059669; width:22px; height:22px;"></i>
          </div>
        </div>

        <!-- 4. Assigned Card -->
        <div class="kpi-mini-card" style="border-left: 4px solid #d97706; padding:18px 20px;">
          <div class="kpi-mini-info">
            <span class="kpi-mini-title" style="font-size:0.72rem; font-weight:700; color:#64748b;">ASSIGNED (CUSTODY)</span>
            <span class="kpi-mini-val" style="color:#d97706; font-size:1.65rem; font-weight:800; margin:4px 0;">${assignedCount}</span>
          ...
          <div style="width:46px; height:46px; border-radius:10px; background:#fffbeb; display:flex; align-items:center; justify-content:center;">
            <i data-lucide="user-check" style="color:#d97706; width:22px; height:22px;"></i>
          </div>
        </div>
      ...
      <div class="dashboard-row-2" style="margin-bottom: 22px;">
        ...
        <div class="fams-card" style="margin-bottom: 0;">
          <div class="card-header-flex">
            <div class="card-header-title">
              <i data-lucide="map-pin" style="color:#4f46e5; width:16px; height:16px;"></i>
              <h3 style="font-size:0.92rem;">LOCATION / PLANT WISE ASSET DISTRIBUTION</h3>
            </div>
            <span style="font-size:0.75rem; color:#64748b; font-weight:600;">Live Distribution</span>
          </div>
              ...
              return `
                <div>
                  <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.82rem; margin-bottom:6px;">
                    <span style="font-weight:600; color:#0f172a;">${loc.shortName}</span>
                    <div>
                      <strong style="color:#4f46e5; font-size:0.9rem;">${loc.count} Assets</strong>
                      <span style="color:#64748b; font-size:0.75rem; margin-left:8px;">(${formatCurrency(loc.value)})</span>
                    </div>
                  </div>
                  <div style="width:100%; height:8px; background:#f1f5f9; border-radius:6px; overflow:hidden;">
                    <div style="width:${Math.max(pct, loc.count > 0 ? 8 : 0)}%; height:100%; background:linear-gradient(90deg, #4f46e5, #0284c7); border-radius:6px; transition:width 0.4s ease;"></div>
                  </div>
                </div>
              `;
            }).join('')}
        ...
        <div class="fams-card" style="margin-bottom: 0;">
          <div class="card-header-flex">
            <div class="card-header-title">
              <i data-lucide="tag" style="color:#0284c7; width:16px; height:16px;"></i>
              <h3 style="font-size:0.92rem;">ASSET CATEGORY BREAKDOWN</h3>
            </div>
            <span style="font-size:0.75rem; color:#64748b; font-weight:600;">Portfolio Summary</span>
          </div>
            ...
            ${categoryStats.map(cat => `
              <div style="display:flex; align-items:center; justify-content:space-between; padding:10px 14px; background:#f8fafc; border-radius:8px; border:1px solid #e2e8f0;">
                <div style="display:flex; align-items:center; gap:10px;">
                  ...
                  <div>
                    <div style="font-weight:700; font-size:0.83rem; color:#0f172a;">${cat.name}</div>
                    <span style="font-size:0.72rem; color:#64748b;">Code: <strong>${cat.code}</strong></span>
                  </div>
                </div>
                <div style="text-align:right;">
                  <div style="font-weight:700; color:#0f172a; font-size:0.88rem;">${cat.count} Units</div>
                  <div style="font-size:0.74rem; font-weight:600; color:#4f46e5;">${formatCurrency(cat.value)}</div>
                </div>
              </div>
            `).join('')}
          </div>
